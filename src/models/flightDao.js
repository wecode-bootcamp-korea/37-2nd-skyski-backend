...
const FlightOrdering = Object.freeze({
  minPrice: 'f.price + f2.price',
  minDuration: 'f.duration + f2.duration',
});

const getOneWayTripFlights = async (queryParams) => {
  const {
    limit = 30,
    offset = 0,
    sort = 'minPrice',
  ...
  const sql = `
    SELECT * 
    FROM flights f
    INNER JOIN route r ON r.id = f.route_id
    WHERE r.departure = ${Departure[departure]}
      AND r.arrival = ${Arrival[arrival]}
      AND date_format(f.departure_date, '%Y-%m-%d') = ${departureDate}
      AND date_format(f.arrival_date, '%Y-%m-%d') = ${arrivalDate}
    ORDER BY ${FlightOrdering[sort]}
  `;

  return await database.query(sql);
};

const getRoundTripFlights = async (queryParams) => {
  const {
    limit,
    offset,
    sort,
...
//     query.flightSeatClass,
//     query.departureTime1,
//     query.departureTime2,
//     query.airline,
//     query.airline,
//   ]);
