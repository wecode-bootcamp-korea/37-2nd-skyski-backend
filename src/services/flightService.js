...
const getFlights = async (queryParams) => {
  const { roundTrip } = queryParams;

  let result;

  if (roundTrip === 'T') {
    result = await flightDao.getOneWayTripFlights(queryParams);
  } else {
    result = await flightDao.getRoundTripFlights(queryParams);
  }

  const result = await flightDao.getFlights(query);
};
